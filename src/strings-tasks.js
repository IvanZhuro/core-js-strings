...
function reverseWords(str) {
  ...
  for (let index = 0; index < strReverse.length; index += 1) {
    strReverse[index] = strReverse[index].split('').reverse().join('');
  }
...
function invertCase(str) {
  ...
  for (let index = 0; index < str.length; index += 1) {
    if (str.charCodeAt(index) >= 65 && str.charCodeAt(index) <= 90) {
      strInvert.push(str.charAt(index).toLowerCase());
    } else if (str.charCodeAt(index) >= 97 && str.charCodeAt(index) <= 122) {
      strInvert.push(str.charAt(index).toUpperCase());
    } else strInvert.push(str.charAt(index));
  }
  return strInvert.join('');
}
...
function extractNameFromTemplate(value) {
  ...
  return array[1].concat(' ', array[2].slice(0, -1));
}
